...
// Bitonic Sort for CPU using pthread
...
void* bitonicSortThread(void* args) 
{
    SortArgs* sortArgs = (SortArgs*)args;
    int* arr = sortArgs->arr;
    int n = sortArgs->n;
    int k = sortArgs->k;
    int j = sortArgs->j;
...
// 修改后的 bitonicSortCPU 函数
void bitonicSortCPU(int* arr, int n, int threadCount) 
{
    pthread_t* threads = new pthread_t[threadCount];
    SortArgs* sortArgs = new SortArgs[threadCount];
    ...
    for (int k = 2; k <= n; k *= 2) 
    {
        for (int j = k / 2; j > 0; j /= 2) 
        {
            ...
            {
                sortArgs[t] = { arr, n, k, j };
                pthread_create(&threads[t], nullptr, bitonicSortThread, (void*)&sortArgs[t]);
            }
            for (int t = 0; t < threadCount; t++) 
            {
...
{   
    ...
    if (argc < 2) {
        std::cout << "Usage: ./<program> <x for 2^x>" << std::endl;
        return 1;
    }
    int size = atoi(argv[1]);

    // std::cout << "\n\nEnter the X for 2^X size of the array (X<=31):\n ";
    // std::cin>>size;
    ...
    if (!isSorted(arr, size))
        std::cout << "!!! FAIL GPU!!!" << std::endl;
        // std::cout << "\nSUCCESFULLY SORTED GPU ARRAY" << std::endl;
    // else
   
    if (!isSorted(carr, size))
        std::cout << "!!! FAIL CPU!!!" << std::endl;
        // std::cout << "SUCCESFULLY SORTED CPU ARRAY" << std::endl;
    // else
